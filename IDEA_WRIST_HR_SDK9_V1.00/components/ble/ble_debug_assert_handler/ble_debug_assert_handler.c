...
#define check_and_backup(dst,src,size,limit)                                                \
                    do {                                                                    \
                        if(dst + size > limit) {                                            \
                            DEBUG_ERROR("backup data exceed size on line %d",__LINE__); \
                            return;                                                         \
                        }                                                                   \
                        memcpy(dst,src,size);                                               \
                        dst += size;                                                        \
...
#define check_and_restore(dst,src,size,limit)                                                 \
                    do {                                                                      \
                        if(src + size > limit) {                                              \
                            DEBUG_ERROR("restore data exceed size on line %d",__LINE__);  \
                            goto ERROR_HANDLE;                                                \
                        }                                                                     \
                        memcpy(dst,src,size);                                                 \
                        src += size;                                                          \
...
uint32_t restore_backup_info_from_flash(void)
{
    ...
    flash_page_data = FLASH_PAGE_STORE_TIME;
    ...
    if(address) {
        ...
        if((header & 0xFFFF0000U) == BLE_FLASH_MAGIC_NUMBER) {
            ...
            if(cal_crc == crc_header) {
                ...
                DEBUG_PRINT( "len: %d:date: %d-%d-%d",sleep_setting_event_head->length, sleep_setting_event_head->Date.date.year,sleep_setting_event_head->Date.date.month, 
sleep_setting_event_head->Date.date.day);



                for(int ii = 0; ii < word_count+FLASH_PAGE_HEADER_LEN; ii ++)
                    DEBUG_PRINT("restore[%d]:0x%08x\n",ii,info_to_restore[ii]);
#endif
                //erase the page
                ble_flash_page_erase(FLASH_PAGE_STORE_TIME);
...
static void pre_restart_info_store(backup_info_restart_type_t restart_type)
{
    ...
    DEBUG_PRINT("word to backup is %d",word_count);
...
#ifdef DEBUG_LOG
        DEBUG_PRINT("backup[%d]:0x%08x",i,info_to_backup[i]);
#endif
    }
