...
{
    // 先定义数据alarm_mem[]，用于存放FLASH中读取出来的数据。
...
////    err_code = bd_flash_read_page(FLASH_PAGE_ALARM_SETTINGS,alarm_mem,&read_word_len);
    err_code = ble_flash_page_read(FLASH_PAGE_ALARM_SETTINGS,alarm_mem,&read_word_len);
    if(err_code == NRF_SUCCESS) {
        alarm_count_stored = (uint8_t)alarm_mem[0];
        // 如果读取出来的闹钟数量与设置的闹钟数量相等
    ...
    if(check_is_flash_erased(alarms_addr,(FLASH_PAGE_HEADER_LEN + 1 + sizeof(alarm_union_t)/sizeof(uint32_t) * MAX_ALARM_NUM)) != NRF_SUCCESS) { //page not erased
        // 如未檫除则需要调用ble_flash_page_delay_erase()进行异步存储，
        // 这样做的原因是因为檫除操作比较耗时，如果同步去檫除时，
        // 会导影响蓝牙程序，因此这个做了一个异步操作，详细参与异步存储。
...
{
    // 先定义存储读取出来的数据的临时数组alarm_mem[]
    uint32_t    alarm_mem[1 + sizeof(alarm_union_t)/sizeof(uint32_t) * MAX_ALARM_NUM];

    uint32_t    err_code;
    uint8_t     read_word_len = 1 + sizeof(alarm_union_t)/sizeof(uint32_t) * MAX_ALARM_NUM;

    // 读取相应FLASH数据至alarm_mem[]中。如果读取失败，则清空ALARM数据，
////    err_code = bd_flash_read_page(FLASH_PAGE_ALARM_SETTINGS,alarm_mem,&read_word_len);
    err_code = ble_flash_page_read(FLASH_PAGE_ALARM_SETTINGS,alarm_mem,&read_word_len);
    if(err_code != NRF_SUCCESS) {
        alarm_num = 0;
...
int set_user_profile(userprofile_union_t *profile)
{
    ...
    // 此处应有蓝牙协议直接解析出来，但蓝牙协议中没有设置步幅，先暂时默认，后面可更改协议
    user_profile.bit_field.stride = 14;
    
    
#ifdef DEBUG_LOG1

    DEBUG_PRINT("s%d a:%d h%d w%d stride:%d\r\n",user_profile.bit_field.gender,user_profile.bit_field.age,user_profile.bit_field.hight,user_profile.bit_field.weight，user_profile.bit_field.stride);
#endif
...
////    err_code = bd_flash_read_page(FLASH_PAGE_USER_PROFILE,&origin_profile,&read_word_len);
    err_code = ble_flash_page_read(FLASH_PAGE_USER_PROFILE,&origin_profile,&read_word_len);
...
////    err_code = bd_flash_read_page(FLASH_PAGE_USER_PROFILE,&user_profile.data,&read_word_len);
    err_code = ble_flash_page_read(FLASH_PAGE_USER_PROFILE,&user_profile.data,&read_word_len);
    if((err_code != NRF_SUCCESS) || (read_word_len != 1)) {
        DEBUG_ERROR("read user profile fail reset to default value");
        ...
        user_profile.bit_field.stride   = (uint8_t)DEFAULT_STRIDE;
    }
}

uint8_t get_user_stride(void)
{
    uint8_t user_stride = 0;
    
    user_stride = 5 * user_profile.bit_field.stride;
    DEBUG_ERROR("get_user_stride() -->> user_stride is %d\r\n",user_profile.bit_field.stride);
    return user_stride;
}
...
////    err_code = bd_flash_read_page(FLASH_PAGE_DAILY_TARGET,&daily_step_target,&read_word_len);
    err_code = ble_flash_page_read(FLASH_PAGE_DAILY_TARGET,&daily_step_target,&read_word_len);
...
{
#ifdef DEBUG_LOG
    DEBUG_ERROR("flash_page_num:%d ready to sync\r\n",flash_page_num);
#endif
