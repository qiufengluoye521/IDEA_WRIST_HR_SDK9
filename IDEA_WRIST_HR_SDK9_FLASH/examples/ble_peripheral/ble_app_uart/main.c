...
extern uint8_t write_flash_page[256];
...
//////    
    ...
    nrf_delay_ms(1000);
    SpiFlash_Write_Data("123",0,0,0,3);
    nrf_delay_ms(100);
...
//////#endif          // #ifdef OLED
    ...
    //spi_flash_init();
    Spi_Flash_Init();
    //SPI_FLASH_WriteEnable();                        // 写、擦除等操作之前先使能
    SpiFlash_Write_Enable();
    nrf_delay_ms(1);
    //SPI_FLASH_Erase_Sector(0);                      // 擦除sector 0
    SPIFlash_Erase_Sector(0,0);
    nrf_delay_ms(1000);                             // Sector Erase time: 100ms typical
    SpiFlash_Write_Enable();
    nrf_delay_ms(200);                              // 时间等确认
    //SpiFlash_Write_Page("4321", 16, 4 , 0);
    ...
    {
        write_flash_page[i] = Num_0824[i];
    }
    SpiFlash_Write_Page(write_flash_page,0,128);
    nrf_delay_ms(3);
    SpiFlash_Write_Enable();
    nrf_delay_ms(200);                              // 时间等确认
    for(uint16_t i=0;i<128;i++)
    {
        write_flash_page[i] = Num_0824[128+i];
    }
    SpiFlash_Write_Page(write_flash_page,0,128);
    nrf_delay_ms(3);
    ...
    for(uint16_t i=0;i<128;i++)
    {
        write_flash_page[i] = Num0805_battery_blutooth[i];
    }
    SpiFlash_Write_Page(write_flash_page,1,128);
    nrf_delay_ms(3);
    SpiFlash_Write_Enable();
    nrf_delay_ms(200);                              // 时间等确认
    for(uint16_t i=0;i<128;i++)
    {
        write_flash_page[i] = Num0805_battery_blutooth[128+i];
    }
    SpiFlash_Write_Page(write_flash_page,1,128);
    nrf_delay_ms(3);
    ...
    for(uint16_t i=0;i<128;i++)
    {
        write_flash_page[i] = step_distence_calorie[i];
    }
    SpiFlash_Write_Page(write_flash_page,2,128);
    nrf_delay_ms(3);
    SpiFlash_Write_Enable();
    nrf_delay_ms(200);                              // 时间等确认
    for(uint16_t i=0;i<128;i++)
    {
        write_flash_page[i] = step_distence_calorie[128+i];
    }
    SpiFlash_Write_Page(write_flash_page,2,128);
    nrf_delay_ms(3);
    ...
    for(uint16_t i=0;i<128;i++)
    {
        write_flash_page[i] = heart_rate[i];
    }
    SpiFlash_Write_Page(write_flash_page,3,128);
    nrf_delay_ms(3);
    SpiFlash_Write_Enable();
    nrf_delay_ms(200);                              // 时间等确认
    for(uint16_t i=0;i<128;i++)
    {
        write_flash_page[i] = heart_rate[128+i];
    }
    SpiFlash_Write_Page(write_flash_page,3,128);
    nrf_delay_ms(3);
    ...
    for(uint16_t i=0;i<128;i++)
    {
        write_flash_page[i] = Haterate[i];
    }
    SpiFlash_Write_Page(write_flash_page,4,128);
    nrf_delay_ms(3);
    SpiFlash_Write_Enable();
    nrf_delay_ms(200);                              // 时间等确认
    for(uint16_t i=0;i<128;i++)
    {
        write_flash_page[i] = Haterate[128+i];
    }
    SpiFlash_Write_Page(write_flash_page,4,128);
    nrf_delay_ms(3);
    ...
    for(uint16_t i=0;i<128;i++)
    {
        write_flash_page[i] = lowpower[i];
    }
    SpiFlash_Write_Page(write_flash_page,5,128);
    nrf_delay_ms(3);
    SpiFlash_Write_Enable();
    nrf_delay_ms(200);                              // 时间等确认
    for(uint16_t i=0;i<128;i++)
    {
        write_flash_page[i] = lowpower[128+i];
    }
    SpiFlash_Write_Page(write_flash_page,5,128);
    nrf_delay_ms(3);
    ...
    for(uint16_t i=0;i<128;i++)
    {
        write_flash_page[i] = charging[i];
    }
    SpiFlash_Write_Page(write_flash_page,6,128);
    nrf_delay_ms(3);
    SpiFlash_Write_Enable();
    nrf_delay_ms(200);                              // 时间等确认
    for(uint16_t i=0;i<128;i++)
    {
        write_flash_page[i] = charging[128+i];
    }
    SpiFlash_Write_Page(write_flash_page,6,128);
    nrf_delay_ms(3);
    ...
    for(uint16_t i=0;i<128;i++)
    {
        write_flash_page[i] = chargfull[i];
    }
    SpiFlash_Write_Page(write_flash_page,7,128);
    nrf_delay_ms(3);
    SpiFlash_Write_Enable();
    nrf_delay_ms(200);                              // 时间等确认
    for(uint16_t i=0;i<128;i++)
    {
        write_flash_page[i] = chargfull[128+i];
    }
    SpiFlash_Write_Page(write_flash_page,7,128);
    nrf_delay_ms(3);
    ...
    OLED_ShowString(0,0,"HELLO",7);
    nrf_delay_ms(1000);
    SpiFlash_Read(spim_dma_rxbuf,0,128);
    OLED_DrawBMP(0,0,64,2,spim_dma_rxbuf+4);
    SpiFlash_Read(spim_dma_rxbuf,0,128);
    OLED_DrawBMP(0,2,64,4,spim_dma_rxbuf+4);
    nrf_delay_ms(1000);

    SpiFlash_Read(spim_dma_rxbuf,1,128);
    OLED_DrawBMP(0,0,64,2,spim_dma_rxbuf+4);
    SpiFlash_Read(spim_dma_rxbuf,1,128);
    OLED_DrawBMP(0,2,64,4,spim_dma_rxbuf+4);
    nrf_delay_ms(1000);    
    ...
    SpiFlash_Read(spim_dma_rxbuf,2,128);
    OLED_DrawBMP(0,0,64,2,spim_dma_rxbuf+4);
    SpiFlash_Read(spim_dma_rxbuf,2,128);
    OLED_DrawBMP(0,2,64,4,spim_dma_rxbuf+4);
    nrf_delay_ms(1000);    
    ...
    SpiFlash_Read(spim_dma_rxbuf,3,128);
    OLED_DrawBMP(0,0,64,2,spim_dma_rxbuf+4);
    SpiFlash_Read(spim_dma_rxbuf,3,128);
    OLED_DrawBMP(0,2,64,4,spim_dma_rxbuf+4);
    nrf_delay_ms(1000);    
    
    SpiFlash_Read(spim_dma_rxbuf,4,128);
    OLED_DrawBMP(0,0,64,2,spim_dma_rxbuf+4);
    SpiFlash_Read(spim_dma_rxbuf,4,128);
    OLED_DrawBMP(0,2,64,4,spim_dma_rxbuf+4);
    nrf_delay_ms(1000);    
    ...
    SpiFlash_Read(spim_dma_rxbuf,5,128);
    OLED_DrawBMP(0,0,64,2,spim_dma_rxbuf+4);
    SpiFlash_Read(spim_dma_rxbuf,5,128);
    OLED_DrawBMP(0,2,64,4,spim_dma_rxbuf+4);
    nrf_delay_ms(1000);    
    
    SpiFlash_Read(spim_dma_rxbuf,6,128);
    OLED_DrawBMP(0,0,64,2,spim_dma_rxbuf+4);
    SpiFlash_Read(spim_dma_rxbuf,6,128);
    OLED_DrawBMP(0,2,64,4,spim_dma_rxbuf+4);
    nrf_delay_ms(1000);    
    
    SpiFlash_Read(spim_dma_rxbuf,7,128);
    OLED_DrawBMP(0,0,64,2,spim_dma_rxbuf+4);
    SpiFlash_Read(spim_dma_rxbuf,7,128);
    OLED_DrawBMP(0,2,64,4,spim_dma_rxbuf+4);
    nrf_delay_ms(1000);    
