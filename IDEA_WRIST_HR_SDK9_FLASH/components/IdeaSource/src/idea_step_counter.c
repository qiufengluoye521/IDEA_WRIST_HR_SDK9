...
void handle_sleep_event(algorithm_event_t *event)
{
    ...
    DEBUG_PRINT( "ts: %lld | %d ", algorithm_timestamp, event->sleep.starting_time_stamp);
#ifdef DEBUG_LOG

    UTCTimeStruct * tm = get_wall_clock_time();
    DEBUG_PRINT("system_clock: [%d/%02d/%02d %02d:%02d:%02d]\r\n",tm->year,tm->month,tm->day,tm->hour,tm->minutes,tm->seconds);
#endif
...
void health_algorithm_cb_implement(algorithm_event_t *event, void* user_data)
{
    ...
    for(j = 0;(j<15) && (inner_step_count < 30000);++j,inner_step_count++) {
        ...
        if(inner_step_count) {
            ...
            init_time += 60;
                ...
                DEBUG_PRINT( "inner step_count is %d",inner_step_count);
            #endif
        }
    }
    ...
    DEBUG_PRINT("Mock generate %d events",count);
#else 
...
#ifdef DEBUG_ACC
    DEBUG_PRINT("f:%d a:%d h%d w%d\r\n",isfemale,age,hight_cm,weight_kg);
#endif
#ifdef DEBUG_LOG

    UTCTimeStruct * tm = get_wall_clock_time();
    DEBUG_PRINT("new day: %d-%d-%d=%d:%d:%d\r\n",tm->year, tm->month, tm->day, tm->hour, tm->minutes, tm->seconds);
#endif
...
void LIS3DH_INT1_event_handler(uint8_t pin_no,uint8_t button_action)
{
    u8_t int1res;
    ...
    for(index = 0x07; index < 0x3D; index ++) {
        ...
        DEBUG_PRINT("config add:0x%x==v:0x%x \n",index,value);
    }
#endif
...
void LIS3DH_INT2_event_handler(uint8_t pin_no,uint8_t button_action)
{
    u8_t  click_res;
    spi_master_enable(SPI0);
...
void tap_timer_handler(void * val)
{
    ...
    DEBUG_PRINT("tap_timer_handler Taped_cnt: %d \n",Taped_cnt);
...
{
//    DEBUG_ERROR("Enter sensor_timer_handle fun \r\n");
...
//    uint8_t regester_value;
    hal_acc_GetFifoData(&transfer_size);
//    DEBUG_PRINT("transfer_size is: %d, \r\n",transfer_size);
...
//            DEBUG_PRINT("accX is: %d,accY is: %d,accZ is: %d,size is: %d, \r\n",accX,accY,accZ,transfer_size);
...
{
    DEBUG_PRINT( "ts: %lld : RTC: %d(%lld)", algorithm_timestamp, get_wall_clock_time_counter(),
...
//    DEBUG_PRINT( "ts: %lld : RTC: %d", algorithm_timestamp, get_wall_clock_time_counter());
...
#ifdef DEBUG_ACC
            DEBUG_ERROR("mSleep_mode%d,timer%d;/n",mSleep_mode, sensor_timer_status);
#endif
        }
    }
    if(tm->minutes%15 == 0) {
...
#ifdef DEBUG_ACC
        DEBUG_PRINT("algorithm_timestamp :RTC: %lld || %d \n",algorithm_timestamp, get_wall_clock_time_counter());
#endif
...
void click_algorithm_cb_implement(click_event_t *event, void* user_data)
{
    ...
    DEBUG_PRINT("click_algorithm_cb_implement \n");
...
//    err_code = register_click_algorithm_callback(click_algorithm_cb_implement,(void *)NULL);
    register_click_button_callback(click_algorithm_cb_implement);
}

uint32_t get_calory(void)
{
    uint32_t calory = 0;
    
    calory  = (uint32_t)mSportsData.bits.Calory;
    return calory;
}
