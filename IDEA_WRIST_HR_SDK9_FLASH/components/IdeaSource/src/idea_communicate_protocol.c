...
#include "core_cm0.h"
#include "ble_hci.h"
#include "idea_stack_dump_repoter.h"
#include "ble_debug_assert_handler.h"
#include "hal_acc.h"
#include "idea_data_manager.h"
#include "idea_bluetooth_log.h"
...
void ota_time_out_handle(void * context)
{
    ...
    DEBUG_PRINT("will switch to OTA mode\r\n");
...
static uint32_t L1_resend_package(L1_Send_Content * content)
{
    DEBUG_PRINT("will resend a package\r\n");
...
static void response_package_handle(uint16_t sequence_id,uint8_t crc_check)
{
    ...
    if(!current_package_wait_response) {
        DEBUG_PRINT(" no package wait res\r\n");
    ...
    if(current_package_wait_response->sequence_id == sequence_id ) {
        ...
        if( crc_check == CRC_SUCCESS) {
            ...
            DEBUG_PRINT("get response show crc succ\r\n");
            ...
            DEBUG_PRINT("get response show crc fail\r\n");
            ...
            if(current_package_wait_response->resendCount >= 3) {
                ...
                DEBUG_PRINT("response crc err resend\r\n");
        ...
        DEBUG_PRINT("receive a package with wrong sequesnce id\r\n");
    }
}
...
void schedule_async_send(void * contenxt,SEND_TASK_TYPE_T task)
{
    ...
    *************************************************************************/
    ...
    if(task == TASK_DATA) { //may be comes from data (send & resend) or (send complete callback)
        ...
        DEBUG_PRINT("call schedule_async_send with wrong para\r\n");
...
static uint32_t  resolve_HealthData_command(uint8_t key,const uint8_t * value ,uint16_t length)
{
    ...
    DEBUG_PRINT("key %d:len:%d - %d",key,length,value[0]);
    //    char str[32];
    ...
    switch(key) {
        ...
        case KEY_SET_STEPS_NOTIFY: //set auto notify
            if(length == 1) {
#ifdef DEBUG_LOG
                DEBUG_PRINT("KEY_SET_STEPS_NOTIFY value:%d\n",value[0]);
                //   char str[32];
                //   sprintf(str,"key %d:len:%d - %d\t",key,length,value[0]);
                //   simple_uart_putstring((const uint8_t *)str);
#endif
...
//                        set_global_calories_today(daily_calory*10);
            ...
            case KEY_LATEST_DATA_SYNC:
                if(length == 10){
                    ...
                    DEBUG_PRINT("calories:    0x%x",calories);
...
#ifdef DEBUG_LOG
//    DEBUG_PRINT("notify: key %d:len:%d -v: %d",key,length,value[0]);
    //     char str[32];
...
#ifdef DEBUG_LOG
    DEBUG_PRINT("SETTINGS---key %d:len:%d - %d\n",key,length,value[0]);
    //    char str[32];
...
#ifdef DEBUG_LOG
    {
//        DEBUG_PRINT("resolve_private_bond_command:key[%d]\n",key);
        //   char str[128];
...
// edit temporary for debug
////                if( batt < PRIVATE_BOND_POWER_LIMIT ){
////#ifdef DEBUG_LOG
////                    DEBUG_PRINT("Battery Level is too Low for BONDING(%d)\n",batt);
////#endif                
////                    bond_fail_event_dispatch();
////                    break;
////                }
...
#ifdef DEBUG_LOG
            DEBUG_PRINT("Battery Level is too Low for OTA(%d)\n",batt);
#endif                
...
#ifdef DEBUG_LOG
    DEBUG_PRINT("notify: key %d:len:%d -v: %d\n",key,length,value[0]);
#endif
...
void schedule_crc_error_handle(void * para,uint16_t event_size)
{
    ...
    DEBUG_PRINT("received data crc check error\n");
}
...
{
    // 先判断是否接收等待定时器是否有启动，如果定时器启动，则停止其，表示在定时到达时间内接收到了数据。
    ...
    switch (receive_state) {
        case WAIT_START: // we need package start
            ...
            // 根据协议层数据包结构（2.3.2）第二第三个byte表示Payload Length ，需要接收数据的长度
            ...
            if(length_to_receive <= 0) { // just one package
                // 如无剩余帧，则判断是否为应答包，是的话则进行等待应答信号的回调处理，并将receive_state设置为WAIT_START。
                // 如果为非应答包，则将receive_state设置为MESSAGE_RESOLVE
                ...
                if(inner_version.version_def.ack_flag == RESPONSE_PACKAGE) { //response package
                    ...
                    DEBUG_PRINT("receive a ack package\n");
                ...
                if(L1_crc_check(crc16_value,received_buffer+L1_HEADER_SIZE,(received_buffer[L1_PAYLOAD_LENGTH_LOW_BYTE_POS] | (received_buffer[L1_PAYLOAD_LENGTH_HIGH_BYTE_POS] << 8))) == NRF_SUCCESS) { //check crc for received package
                    //DEBUG_PRINT("will send success response\n");
                    //send response
                    ...
                    DEBUG_PRINT("will send crc fail response\n");
                    //send response
                ...
                // 计算是否有接收剩余的数据帧。如有剩余帧，则设置状态为WAIT_MESSAGE，等待接收剩余帧
        ...
        case WAIT_MESSAGE:
            ...
            received_content_length += length;
            length_to_receive -= length;
            // 先判断实际接收的字节数是否等于协议包总字节数，如果未接收完成，则继续接收，并启动超时接收定时器(else)
            // 如果接收完成，则再次判断当前接收的数据包（注意，是数据包，而不是数据帧）是否为应答包，
            // 是的话则进行等待应答信号的回调处理，并将receive_state设置为WAIT_START。
            if(length_to_receive <= 0) {
                ...
                // 如果为非应答包，则将receive_state设置为MESSAGE_RESOLVE，接着进行CRC检验，并且根据校验结果返回ACK或NACK，并进行L2层数据解析。此部分操作与在WAIT_START一样
                receive_state = MESSAGE_RESOLVE;
                received_content_length = 0;
            ...
            break;
        // 当状态为MESSAGE_RESOLVE时，则表示目标接收的协议包应该为应答包，接收到数据后进行相应的回调处理。
        // 这个地方好像永远都不会进入的，因为在后面的L2层数据解析的最后，会将receive_state重新设置为WAIT_START，
        // CRC校验错误的处理也会重新设置为WAIT_START。
        case MESSAGE_RESOLVE:
            ...
            if(inner_version.version_def.ack_flag == RESPONSE_PACKAGE) { //response package
                DEBUG_PRINT("receive a ack package during MESSAGE_RESOLVE\n");
