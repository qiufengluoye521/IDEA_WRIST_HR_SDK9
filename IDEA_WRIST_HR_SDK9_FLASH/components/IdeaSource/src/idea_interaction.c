...
#ifdef OLED
        menu_display(UI_MENU_DISPLAY_CALLING);
#endif          // #ifdef OLED
...
#ifdef OLED
        menu_display(UI_MENU_DISPLAY_ALARM);
#endif          // #ifdef OLED
...
#ifdef OLED
        menu_display(UI_MENU_DISPLAY_LOSE);
#endif          // #ifdef OLED
            if( value ) {
...
#ifdef OLED
        menu_display(UI_MENU_DISPLAY_SWITCH);
#endif          // #ifdef OLED
            if( value ) {
...
#ifdef OLED
        menu_display(UI_MENU_DISPLAY_TARGET);
#endif          // #ifdef OLED
...
#ifdef OLED
        menu_display(UI_MENU_DISPLAY_SLEEP);
#endif          // #ifdef OLED
            show_is_in_sleep_mode();
            } else {
#ifdef OLED
        menu_display(UI_MENU_DISPLAY_STEP);
#endif          // #ifdef OLED
...
//    DEBUG_PRINT("show_step_task_complete_status:[%d]\n",temp);
#endif
}
...
static void show_is_in_sleep_mode(void)
{
    ...
    //    simple_uart_putstring((const uint8_t *)str);
//    DEBUG_PRINT("show_is_in_sleep_mode\n");
#endif
}
