...
void update_last_time_sleep_mode(void)
{
    ...
    DEBUG_PRINT( "%d, day:%d", last_sleep_setting_event_head.length, last_sleep_setting_event_head.Date.date.day);
    DEBUG_PRINT( "flag:%d,mode: %d,ts %d", last_sleep_setting_event.bits.sleeping_flag,last_sleep_setting_event.bits.mode, last_sleep_setting_event.bits.timeStamp);
}
...
{
    DEBUG_PRINT( "length %d, day:%d", last_sleep_setting_event_head.length, last_sleep_setting_event_head.Date.date.month, last_sleep_setting_event_head.Date.date.day);
    DEBUG_PRINT( "flag:%d,mode: %d,ts %d", last_sleep_setting_event.bits.sleeping_flag,last_sleep_setting_event.bits.mode, last_sleep_setting_event.bits.timeStamp);
...
#ifdef DEBUG_LOG
    DEBUG_PRINT(" sleeping flag: %d",curr_sleep_flag);
#endif
}
...
#ifdef DEBUG_PHILL
                DEBUG_PRINT("Auto Wakeup, after 4 hours!\n");
#endif
                //auto wakeup
                two_Double_tap_handle();
                auto_wakeup_steps = 0;
...
#ifdef DEBUG_ACC
    DEBUG_PRINT( "send success,still have: %d\r\n", manual_sleep_events_head->length);
#endif
...
#ifdef DEBUG_ACC
    DEBUG_PRINT("length:%d", manual_sleep_events_head->length);
    for(int ii = 0; ii <= (SPORT_HEAD_LEN + ((manual_sleep_events_head->length-1) <<2) + 3); ii ++)
    {
       DEBUG_PRINT( "%x", sleep_events_buff[ii]);
    }

    DEBUG_PRINT("date:%d-%d-%d", manual_sleep_events_head->Date.date.year, manual_sleep_events_head->Date.date.month, manual_sleep_events_head->Date.date.day);

    DEBUG_PRINT("sleeping_flag: %d", manual_sleep_event.bits.mode);
    DEBUG_PRINT("timeStamp:%d", manual_sleep_event.bits.timeStamp);
#endif
...
#ifdef DEBUG_LOG        
        DEBUG_PRINT("curr_sleep_flag:%d", curr_sleep_flag);
        DEBUG_PRINT("length:%d", manual_sleep_events_head->length);
        for(int ii = 0; ii <= (SPORT_HEAD_LEN + ((manual_sleep_events_head->length-1) <<2) + 3); ii ++)
         {
           DEBUG_PRINT( "%d", sleep_events_buff[ii]);
         }
        DEBUG_PRINT("date:%d-%d-%d", manual_sleep_events_head->Date.date.year, manual_sleep_events_head->Date.date.month, manual_sleep_events_head->Date.date.day);

        DEBUG_PRINT("sleeping mode: %d", manual_sleep_event.bits.mode);
        DEBUG_PRINT("sleeping_flag: %d", manual_sleep_event.bits.mode);
        DEBUG_PRINT("timeStamp:%d", manual_sleep_event.bits.timeStamp);
#endif
