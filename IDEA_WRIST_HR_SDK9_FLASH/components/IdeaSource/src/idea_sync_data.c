...
#ifdef DEBUG_LOG
        DEBUG_PRINT("no sleep data in flash for send \n");
#endif
...
#ifdef DEBUG_LOG
    //        DEBUG_PRINT("get_next_sleep_read_group_address \n");;
    DEBUG_PRINT(" g_saved_SleepData.p_read_addr:0x%x  \n",g_saved_SleepData.p_read_addr);
#endif
...
#ifdef DEBUG_LOG
    DEBUG_PRINT(" next_read_address:0x%x  \n",next_read_address);
#endif
...
#ifdef DEBUG_LOG
    //         DEBUG_PRINT("get_next_sleep_write_group_address \n");
    DEBUG_PRINT("p_write_addr:0x%x  \n",g_saved_SleepData.p_write_addr);
#endif
...
#ifdef DEBUG_LOG
        DEBUG_PRINT("need to erase next page: p_write_addr:0x%x \n",g_saved_SleepData.p_write_addr);
#endif
...
#ifdef DEBUG_LOG
    DEBUG_PRINT("next_write_address:0x%x  \n",next_write_address);
#endif
...
#ifdef DEBUG_LOG
        DEBUG_PRINT("no sport data in flash for send \n");
#endif
...
#ifdef DEBUG_LOG
    //        DEBUG_PRINT("get_next_sport_read_group_address \n");
    DEBUG_PRINT(" p_read_addr:0x%x  \n",g_saved_SportsData.p_read_addr);
#endif
...
#ifdef DEBUG_LOG
    //         DEBUG_PRINT("get_next_sport_write_group_address");
...
uint32_t save_sport_group_data(uint8_t counts)
{
    ...
    DEBUG_PRINT("save_sport_group_data \n");
#endif
...
bool send_sport_group_data(void)
{
    ...
    DEBUG_PRINT("send_sport_group_data \n");
#endif
    ...
    int i =0;
    for(i=0; i<9; i++) {
        DEBUG_PRINT("read_address[%d]: 0x%x \n",i,read_address[i]);
    }
#endif
    ...
    DEBUG_PRINT("sentLen hight value %d \n",sentLen);
#endif
...
bool send_sleep_group_data(void)
{
    ...
    DEBUG_PRINT("send_sleep_group_data \n");
#endif
...
bool send_sport_data_from_ram(void)
{
    ...
    DEBUG_PRINT("send_sport_data_from_ram \n");
#endif
...
uint32_t save_sleep_group_data(uint8_t counts)
{
    ...
    DEBUG_PRINT("save_sleep_group_data \n");
#endif
...
void sleep_evt_handler(algorithm_event_t *event)
{
    ...
    DEBUG_PRINT("sleep_head: %d-%d-%d",mSleepHead.Date.date.year, mSleepHead.Date.date.month, mSleepHead.Date.date.day);
    DEBUG_PRINT("mSleepData.bits.timeStamp %d", mSleepData.bits.timeStamp);
    DEBUG_PRINT("sleepStart_time:%d",event->sleep.starting_time_stamp);
    DEBUG_PRINT("sleep_minutes : %d-sleepHead_minutes %d", sleep_minutes, sleepHead_minutes);
#endif
...
void check_sports_data_in_flash(void)
{
    ...
    for( ; page_num < SPORTS_DATA_BASE_PAGE + SPORTS_DATA_PAGE_NUM ; ++page_num) {
        ...
        *****************************************************************/
        ...
        DEBUG_PRINT("sports data paga %d first word :%x,%x,%x,%x",page_num,header[0],header[1],header[2],header[3]);
...
void check_sleep_data_in_flash(void)
{
    ...
    for( ; page_num < SLEEP_DATA_BASE_PAGE + SLEEP_DATA_PAGE_NUM ; ++page_num) {
        ...
        *****************************************************************/
        ...
        memcpy((char *)header,(char *)address,4);
        DEBUG_PRINT("sleep data paga %d first word :%x,%x,%x,%x",page_num,header[0],header[1],header[2],header[3]);
...
{
    // 如果未与手机绑定，不需要发送数据，直接退出 
    if((private_bond_machine != PRIVATE_BOND_SUCCESS) ) {
        return;
    }
    // 如果有数据正在发送，则启动send_data_timer定时器，
    // 每隔5s进入定时器中段一次，重新send_all_data()
    if(sending_flag) {
        start_send_data_timer();
    ...
    // 当sync_process_start_send == false（还未发送开始命令）并且有数据存储在FLASH或RAM中，
    // 则需要发送 同步开始 命令（CMD=0x05，KEY= 0x07）。
    ...
    // FLASH中有睡眠数据存储需要发送，但此时sync_process_start_send == true（已经发送开始命令），
    // 则发送 flash中sleep数据 
    // 通过判断睡眠数据的当前写地址与当前读地址是否相等来判断是否有数据存储在FLASH中。
    // 如果写地址与读地址相等，说明没有数据存储在FLASH中。
    ...
    // RAM中有睡眠数据存储需要发送，此时已发送同步开始命令，且Flash中已无数据需要更新，
    // 则发送 ram中sleep数据
    // 通过判断睡眠数据的结构体mSleepHead.length是否等于0来判断是否有数据需要上传，
    // 当此值不为0时，则说明有数据上传
    ...
    // 有睡眠设置数据存储需要发送，此时已发送同步开始命令，且Flash及RAM中已无数据需要更新，
    // 则发送睡眠设置数据
    // 通过判断manual_sleep_events_head->length是否大于0进行判断。
    // manual_sleep_events_head->length在切换睡眠（应用层）模式的时候进行加1，在发送睡眠数据后的回调函数中作减操作
    // （send_sleep_setting_data_cb）。睡眠模式应该分为两种，一种为BAIDU算法的睡眠，有未睡、浅睡和深睡三种模式，
    // 这种睡眠在全天任何时候可能都会切换；另一种是应用级别的睡眠（即那个22~5点的那个，我们可以修改的算法），
    // 这个睡眠会调用0505指令去告诉上位机进入睡眠或退出睡眠的状态。
    ...
    // FLASH中有sports数据存储需要发送，此时已发送同步开始命令，且睡眠相关内容已无数据需要更新，
    // 则发送Flash中sport相关数据  判断方法与have_sleep_group_to_send()一致。
    ...
    // RAM中有sports  历史  数据存储需要发送，此时已发送同步开始命令，且睡眠相关内容已无数据需要更新，
    // 且Flash中无sports数据需要更新，则发送RAM中sport相关数据
    // 通过判断运动数据的结构体mSportHead.length是否等于0来判断是否有数据需要上传，当此值不为0时，则说明有数据上传。
    ...
    // 检测到有步数需要更新，且RAM中有运动数据需要更新，且前面的数据都不需要更新，则发送RAM中sport相关数据
    ...
    // sync_process_start_send == true（已经发送开始命令），但无数据需要发送，则发送结束命令
