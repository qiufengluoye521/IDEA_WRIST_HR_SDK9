...
void __log2( const char * func, uint32_t line, const char * restrict format, ...);

#if DEBUG_PRINT_ENABLE
    #define DEBUG_PRINT( format, ...) __log2( __func__, __LINE__, format, ##__VA_ARGS__);
#else
    #define DEBUG_PRINT( format, ...)
#endif                                      // #if DEBUG_PRINT_ENABLE

        
#if DEBUG_ERROR_ENABLE
    #define DEBUG_ERROR( format, ...) __log2( __func__, __LINE__, format, ##__VA_ARGS__);
#else
...
//    #define DEBUG_INFO(msg...) __log2( __func__, __LINE__, format, ##__VA_ARGS__);
//#else
